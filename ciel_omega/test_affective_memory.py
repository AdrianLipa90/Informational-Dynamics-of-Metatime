...
def test_1_repeated_alerts_create_candidate():
    ...
    return cand is not None and cand.trace_support_count >= 3 and cand.polarity == 'alert'
...
def test_2_single_episode_no_item():
    ...
    return item is None and len(m5.items) == 0
...
def test_4_retrieval_prefers_high_salience_alignment():
    ...
    return len(results) > 0 and results[0]['item'].affective_key == 'warning risk of harm'
...
def test_5_no_direct_path_to_m6_or_identity_mutation():
    ...
    return identity.phase == phase_before and identity.anchors == anchors_before
