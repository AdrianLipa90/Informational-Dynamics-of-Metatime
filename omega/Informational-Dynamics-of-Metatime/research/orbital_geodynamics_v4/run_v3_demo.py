...
def main():
    ...
    (root/'config'/'sectors_real_v3.json').write_text(json.dumps(payload['sectors'], indent=2), encoding='utf-8')
    (root/'config'/'couplings_real_v3.json').write_text(json.dumps(payload['couplings'], indent=2), encoding='utf-8')

    system = load_system(root/'config'/'sectors_real_v3.json', root/'config'/'couplings_real_v3.json')
    ...
    out = {
        'engine': 'Euler-Berry-Poincare-421-AijTau-global-v3',
        'mesh_inputs': {
    ...
    (out_dir/'v3_demo_results.json').write_text(json.dumps(out, indent=2), encoding='utf-8')
    print(out_dir/'v3_demo_results.json')
