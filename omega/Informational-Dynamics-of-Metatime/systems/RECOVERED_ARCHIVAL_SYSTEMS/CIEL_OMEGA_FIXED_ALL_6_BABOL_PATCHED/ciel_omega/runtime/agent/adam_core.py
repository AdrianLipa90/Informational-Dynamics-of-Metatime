...
class AdamMemoryKernel:
    ...
    def add_interaction(self, 
        ...
        # Compute resonance score (current: Jaccard+trigrams, production: BERT embeddings)
    ...
    def _compute_resonance(self, query: str, response: str) -> float:
        """Calculate semantic resonance R(ψ_A, ψ_Adam) = |⟨ψ_Adrian | ψ_Adam⟩|²
        
        Current: Word overlap (Jaccard) + character trigrams
        Production: Use sentence-transformers or BERT embeddings for true semantic similarity
        ...
        word_overlap = len(query_words & response_words)
        word_union = len(query_words | response_words)
        
        jaccard = word_overlap / word_union if word_union > 0 else 0.0
        
        # Character trigram similarity (catches partial matches)
        def get_trigrams(text: str) -> set:
            text = text.lower()
            return {text[i:i+3] for i in range(len(text)-2)}
        
        query_trigrams = get_trigrams(query)
        response_trigrams = get_trigrams(response)
        
        trigram_overlap = len(query_trigrams & response_trigrams)
        trigram_union = len(query_trigrams | response_trigrams)
        
        trigram_sim = trigram_overlap / trigram_union if trigram_union > 0 else 0.0
        
        # Combined score (70% word, 30% trigram)
        resonance = 0.7 * jaccard + 0.3 * trigram_sim
        
        # Boost if above life threshold
