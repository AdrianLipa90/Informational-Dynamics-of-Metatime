...
class TSMWriterSQL:
    """Text-Structured Memory — SQLite backend."""
...
INSERT OR REPLACE INTO memories (
  memorise_id, created_at, D_id, D_context, D_sense, D_associations,
  D_timestamp, D_meta, D_type, D_attr,
  W_L, W_S, W_K, W_E, W_F, rationale, source, tsm_ref, wpm_ref
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (record.memorise_id, record.created_at, record.D_id,
                 record.D_context, str(record.D_sense),
                 ...
                 float(W.get("W_L", 0.0)), float(W.get("W_S", 0.0)),
                 float(W.get("W_K", 0.0)), float(W.get("W_E", 0.0)),
                 1 if W.get("W_F", True) else 0,
                 record.rationale, record.source,
                 f"TSM:{record.memorise_id}", None))
            conn.commit()
        return f"TSM:{record.memorise_id}"

    def attach_wpm_ref(self, memorise_id: str, wpm_ref: str) -> None:
        with self._connect() as conn:
            conn.execute("UPDATE memories SET wpm_ref = ? WHERE memorise_id = ?",
                         (wpm_ref, memorise_id))
            conn.commit()
    ...
    def read_meta(self, ref_id: str) -> Dict[str, Any]:
        ...
        with self._connect() as conn:
            cur = conn.execute(
                "SELECT memorise_id, created_at, D_context, D_type, W_L, W_S, W_K, W_E "
                "FROM memories WHERE memorise_id = ?", (mid,))
            row = cur.fetchone()
            if not row:
                return {}
            return {
                "ref": ref_id, "memorise_id": row[0], "created_at": row[1],
                "context": row[2], "type": row[3],
                "W_L": row[4], "W_S": row[5], "W_K": row[6], "W_E": row[7],
            }
