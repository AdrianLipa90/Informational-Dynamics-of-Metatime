...
def test_1_repeated_success_creates_candidate():
    ...
    return cand is not None and cand.trace_support_count >= 3
...
def test_2_single_episode_no_item():
    ...
    return item is None and len(m4.items) == 0
...
def test_4_surface_duplicates_same_key():
    ...
    return t1.procedure_key == t2.procedure_key
...
def test_5_retrieval_returns_best_procedure():
    ...
    return len(results) > 0 and results[0]['item'].procedure_key == key
...
def test_7_no_direct_path_to_identity_or_m6():
    ...
    return unchanged_identity and unchanged_m6
