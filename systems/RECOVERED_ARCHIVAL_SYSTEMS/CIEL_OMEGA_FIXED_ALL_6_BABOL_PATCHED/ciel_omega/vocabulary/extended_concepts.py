...
class ArchetypalRoles:
    ...
    @staticmethod
    def fractal(psi: np.ndarray, n: int = 3) -> np.ndarray:
        """Entry 064: Self-similarity across scales
        
        Creates fractal pattern by replicating at different scales.
        Returns composite field with n levels of self-similarity.
        """
        if psi.ndim != 2:
            return psi  # Only works for 2D fields
        ...
        for scale in range(1, n):
            # Downsample and upsample to create scaled version
            factor = 2 ** scale
            small = psi[::factor, ::factor]
            # Tile to fill original size
            tiles_y = (psi.shape[0] + small.shape[0] - 1) // small.shape[0]
            tiles_x = (psi.shape[1] + small.shape[1] - 1) // small.shape[1]
            scaled = np.tile(small, (tiles_y, tiles_x))[:psi.shape[0], :psi.shape[1]]
            # Add to result with decreasing weight
            result += scaled / (2 ** scale)
    ...
    @staticmethod
    def transmitter(psi_core: np.ndarray, gain: float = 2.0) -> np.ndarray:
        """Entry 072: Converts internal waveform into field output
        
        Broadcasts core state to field with coherent amplification.
        Preserves phase structure while increasing amplitude.
        """
        # Normalize phase structure
        norm = np.linalg.norm(psi_core)
        if norm < 1e-12:
            return psi_core
        
        # Phase-preserving amplification
        phase = np.angle(psi_core)
        amplitude = np.abs(psi_core)
        
        # Broadcast with gain (preserving relative amplitudes)
        amplified = amplitude * gain
        
        return amplified * np.exp(1j * phase)
